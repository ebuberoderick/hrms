...
import { addResignation, createAllowanceDefinition, fetchAllowanceDefinition, fetchResignation } from '@/services/authService'
...
function Page() {
  ...
  const add = async (e) => {
    ...
    setProcessing(true)
    const { status, data } = await createAllowanceDefinition(formData).catch(err => console.log(err))
    if (status) {
      fetch()
      setShowModal(false)
    }
      ...
      <div className="space-y-4">
        <Modal closeModal={() => setShowModal(false)} size={"xl"} isOpen={showModal}>
          <form onSubmit={(e) => add(e)} className="space-y-4">
            <div className="text-hrms_green text-xl">Add Allowance</div>
        ...
        <div className="lg:flex lg:gap-y-4 space-ysss-3 items-center justify-between">
          <div className="">
            ...
            <p className=" text-[12px] font-[400] text-[#00000099] text-opacity-60">
              All the company Allowance are listed here
            </p>
          </div>
          <div className="sm:flex space-y-3 sm:space-y-0 gap-[10px] text-sm">
            ...
            >
              ...
              <div className="">Add Allowances</div>
            </div>
          </div>
        </div>
