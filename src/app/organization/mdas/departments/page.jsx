...
import { createMadsDepartment, fetchMadsDepartment } from '@/services/authService'
...
function Page() {
    ...
    const add = async (e) => {
        ...
        const { status, data } = await createMadsDepartment(formData).catch(err => console.log(err))
        if (status) {
            fetch()
            setShowModal(false)
        }
    ...
    const fetch = async () => {
        const { status, data } = await fetchMadsDepartment().catch(err => console.log(err))
        if (status) {
            setRiz(data.data[0]);
        }
        setIsLoading(false)
                ...
                <Modal closeModal={() => setShowModal(false)} size={"xl"} isOpen={showModal}>
                    <form onSubmit={(e) => add(e)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <AppInput name="hod_id" type={"select"} required label="Head of Department" options={[...empl]} />
                            ...
                            <AppInput name="name" type={""} required label="Name Of Department" />
                ...
                <div className="lg:flex space-y-3 items-center justify-between">
                    <div className="">
                        ...
                        <p className=" text-[12px] font-[400] text-[#00000099] text-opacity-60">
                            All the departments  are listed here
                        </p>
                    </div>
                    <div className="sm:flex space-y-3 sm:space-y-0 gap-[10px] text-sm">
                        ...
                        >
                            ...
                            <div className="">Add Department</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="">
                        <table className="w-full divide-y text-xs text-left">
                            <tr className="bg-gray-100">
                                ...
                                <th className="hidden sm:table-cell">Name Of Department</th>
                                ...
                                riz?.data?.map((data, i) => (
                                    <tr key={i} className="">
                                        <td className="flex gap-3 pl-5 py-2">
                                            {data.mdasministry.name}
                                        </td>
                                        <td className="hidden lg:table-cell">{data.code}</td>
