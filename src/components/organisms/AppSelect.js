...
function AppSelect({label,options}) {
    ...
    return (
        <div onMouseLeave={()=> {setShowOption(false),document.getElementById(label === "" ? "-"+figu:label).blur()}} onClick={()=> setShowOption(!showOption)} className='text-[16px] relative rounded-lg'>
            <input value={selected} readOnly id={label === "" ? "-"+figu:label} className='w-full border focus:border-hrms_blue border-hrms_blue placeholder-shown:border-gray-300 p-3 peer outline-none rounded-lg placeholder:text-transparent' placeholder={label} />
            <span className='text-xl absolute top-3 right-3 text-hrms_blue peer-focus:text-hrms_blue peer-placeholder-shown:text-gray-300'><i className="ri-arrow-down-s-line"></i></span>
