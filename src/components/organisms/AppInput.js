...
function AppInput({label,type,required,name}) {
  ...
  return (
    <div className='text-[16px] relative rounded-lg'>
        <input name={name} required={required} type={inputType} className='w-full border focus:border-hrms_blue border-hrms_blue placeholder-shown:border-gray-300 p-3 peer outline-none rounded-lg placeholder:text-transparent' placeholder={label} />
        ...
        {
          type === "password" && (
            <div className='absolute cursor-pointer text-hrms_blue peer-focus:text-hrms_blue peer-placeholder-shown:text-gray-300 right-3 top-3' onClick={() => setInputType(inputType === "password" ? "text":"password")}>
              {
